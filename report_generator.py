...
# 使用モデル: 現時点ではコスト最優先でHaiku 4.5を採用（今後のモデル選定に応じて変更可能）
...
def _format_claim_data(claim_data: dict) -> str:
    lines = [
...
def generate_report(claim_data: dict, report_type: str) -> str:
    ...
    system_prompt = EXTERNAL_SYSTEM_PROMPT if report_type == "社外用" else INTERNAL_SYSTEM_PROMPT
    user_content = "以下のクレーム情報をもとに、報告書を作成してください。\n\n" + _format_claim_data(claim_data)
